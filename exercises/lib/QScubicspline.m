function s=QScubicspline(x,y,xx,type ,der)
% QSCUBICSPLINE compute a cubic spline
% S=QSCUBICSPLINE(X,Y,XX) computes the value at the
% abscissae XX of the natural interpolating cubic
% spline that interpolates the values Y at the nodes X.
% S=QSCUBICSPLINE(X,Y,XX ,TYPE ,DER) if TYPE=0 computes the
% values at the abscissae XX of the cubic spline
...
%
% Authors: A.Quarteroni, F. Saleri, 2006.
%
[n,m]= size(x);
if n == 1
    x = x'; y = y'; n = m;
...
S = S(1:n-1, 4: -1:1); pp = mkpp(x,S); s = ppval(pp ,xx);
return