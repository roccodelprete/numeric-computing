function radice=bisez_ric(funzione,a,b,delta)
...
%            f(a)*f(b)<=0
%   delta    reale non negativo; il massimo
...
if abs(b-a)<=delta+eps*max(abs(a),abs(b))
  ...
  %la radice e' nell'intervallo [a,pmedio]
    radice=bisez_ric(funzione,a,pmedio,delta);
    else
   %la radice e' nell'intervallo [pmedio,b]
    radice=bisez_ric(funzione,pmedio,b,delta);       
    end
end
